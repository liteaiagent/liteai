...
1. **Complex Dialog Internal Navigation:** Some multi-step dialogs (`DialogRewind`, `DialogAgentList`, `DialogPlugin`, `DialogSessionList`, `DialogWorkspace`, and `DialogProvider`) still use `useDialog()` internally for their *sub-navigation* (even though their outer wrappers are compatible with the new modal pane). This internal stack-based routing needs to be replaced with local view state management.
2. **Legacy Cleanup:** Once all internal `useDialog()` usages are removed, the old `<DialogProvider>` in `app.tsx`, `context/dialog.tsx`, and the floating overlay rendering code in `session-layout.tsx` should be fully deleted.
3. **Home Route Modal:** `ProviderSetupBanner` on the home route currently still uses `dialog.push()`. A minimal modal slot must be added to the `HomeRoute` since it lacks the `SessionLayout` where the new pane lives.
...
|---|---|---|
| **User** | `~/.liteai/memory/<agent>/` | All projects |
| **Project** | `<project>/.liteai/memory/<agent>/` | Same project, all sessions |
| **Local** | `<worktree>/.liteai/memory/<agent>/` | Current workspace only |
...
```bash
export LITEAI_DISABLE_AUTO_MEMORY=true
```

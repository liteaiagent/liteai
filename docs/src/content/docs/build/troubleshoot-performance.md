...
Enable verbose telemetry to identify bottlenecks:

```bash
export LITEAI_TELEMETRY=verbose
```

Export traces for analysis:

```bash
export LITEAI_TELEMETRY_EXPORTER=perfetto
```

Open the trace file in `chrome://tracing` or the Perfetto UI to visualize session bottlenecks.

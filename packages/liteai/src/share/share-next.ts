...
export namespace ShareNext {
  ...
  export async function request(): Promise<{
    ...
    if (!active?.active_org_id) {
      const baseUrl = await Config.get().then((x) => x.enterprise?.url ?? "https://liteai.dev")
  ...
  const disabled = process.env.LITEAI_DISABLE_SHARE === "true" || process.env.LITEAI_DISABLE_SHARE === "1"

...
function resolveTheme(theme: ThemeJson, mode: "dark" | "light") {
  ...
  if (hasSelectedListItemText) {
    resolved.selectedListItemText = resolveColor(selectedText)
  } else {
...
export const { use: useTheme, provider: ThemeProvider } = createSimpleContext({
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    const [store, setStore] = createStore({
      ...
      mode: kv.get("theme_mode", props.mode) as "dark" | "light",
      active: (config.theme ?? kv.get("theme", "liteai")) as string,
      ready: false,
    })
    ...
    const values = createMemo(() => {
      return resolveTheme(store.themes[store.active] ?? store.themes.liteai, store.mode)
    })

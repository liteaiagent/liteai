...
function dirId(dir: string) {
  return Project.directoryId(dir)
}
...
describe("migrateFromGlobal", () => {
  test("migrates global sessions on first project creation", async () => {
    // 1. Start with git init but no commits — creates directoryId-based project row
    await using tmp = await tmpdir()
    await $`git init`.cwd(tmp.path).quiet()
    await $`git config user.name "Test"`.cwd(tmp.path).quiet()
    await $`git config user.email "test@liteai.test"`.cwd(tmp.path).quiet()
    const { project: pre } = await Project.fromDirectory(tmp.path)
    const preId = dirId(tmp.path)
    expect(pre.id).toBe(preId)

    // 2. Seed a session under the directory-based ID with matching directory
    const id = uid()
    seed({ id, dir: tmp.path, project: preId })

    // 3. Make a commit so the project gets a real git-based ID
    await $`git commit --allow-empty -m "root"`.cwd(tmp.path).quiet()

    const { project: real } = await Project.fromDirectory(tmp.path)
    expect(real.id).not.toBe(preId)

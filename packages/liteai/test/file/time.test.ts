...
describe("file/time", () => {
    ...
    test("updates timestamp on subsequent reads", async () => {
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          if (!second) throw new Error("expected second")
  ...
  describe("assert()", () => {
    ...
    test("skips check when LITEAI_DISABLE_FILETIME_CHECK is true", async () => {
      await using tmp = await tmpdir()
      const filepath = path.join(tmp.path, "file.txt")
      await fs.writeFile(filepath, "content", "utf-8")
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          const original = Flag.LITEAI_DISABLE_FILETIME_CHECK
          ;(Flag as { LITEAI_DISABLE_FILETIME_CHECK: boolean }).LITEAI_DISABLE_FILETIME_CHECK = true

          try {
            // Should not throw even though file wasn't read
            await FileTime.assert(sessionID, filepath)
          } finally {
            ;(Flag as { LITEAI_DISABLE_FILETIME_CHECK: boolean }).LITEAI_DISABLE_FILETIME_CHECK = original
          }
        },
      })
    })
    ...
    test("detects modification via stat mtime", async () => {
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          if (!originalStat) throw new Error("expected originalStat")

...
import type { Message, Part, Session } from "@liteai-ai/sdk"
import {
  parseShareUrl,
  type ShareData,
...
test("parses valid share URLs", () => {
  expect(parseShareUrl("https://liteai.dev/share/Jsj3hNIW")).toBe("Jsj3hNIW")
...
test("rejects invalid URLs", () => {
  expect(parseShareUrl("https://liteai.dev/s/Jsj3hNIW")).toBeNull() // legacy format
  expect(parseShareUrl("https://liteai.dev/share/")).toBeNull()
  expect(parseShareUrl("https://liteai.dev/share/id/extra")).toBeNull()
...
test("transforms share data to storage format", () => {
  const data: ShareData[] = [
    { type: "session", data: { id: "sess-1", title: "Test" } as Session },
...
test("returns null for invalid share data", () => {
  ...
  expect(transformShareData([{ type: "session", data: { id: "s" } as Session }])).toBeNull() // no messages
})

...
import { createPathHelpers, encodeFilePath, stripQueryAndHash, unquoteGitPath } from "./path"
...
describe("encodeFilePath", () => {
  ...
  describe("macOS paths", () => {
    test("should handle macOS absolute path", () => {
      const macPath = "/Users/kelvin/Projects/liteai/README.md"
      const result = encodeFilePath(macPath)
      const fileUrl = `file://${result}`

      expect(() => new URL(fileUrl)).not.toThrow()
      expect(result).toBe("/Users/kelvin/Projects/liteai/README.md")
    })
  ...
  describe("Windows paths", () => {
    test("should handle Windows absolute path with backslashes", () => {
      const windowsPath = "D:\\dev\\projects\\liteai\\README.bs.md"
      ...
      expect(result).toBe("/D:/dev/projects/liteai/README.bs.md")
    })
    ...
    test("should handle mixed separator path (Windows + Unix)", () => {
      ...
      const mixedPath = "D:\\dev\\projects\\liteai/README.bs.md"
    ...
    test("should NOT create invalid URL like the bug report", () => {
      ...
      // The bug was creating: file://D%3A%5Cdev%5Cprojects%5Cliteai/README.bs.md
      expect(result).not.toContain("%5C") // Should not have encoded backslashes
      expect(result).not.toBe("D%3A%5Cdev%5Cprojects%5Cliteai/README.bs.md")

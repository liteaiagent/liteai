...
describe("prompt-input editor dom", () => {
  ...
  test("length helpers treat breaks as one char and ignore zero-width chars", () => {
    ...
    if (!firstNode) throw new Error("expected first node")
    const secondNode = container.childNodes[1]
    if (!secondNode) throw new Error("expected second node")

    expect(getNodeLength(firstNode)).toBe(2)
    expect(getNodeLength(secondNode)).toBe(1)

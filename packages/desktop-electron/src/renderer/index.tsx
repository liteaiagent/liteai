...
const deepLinkEvent = "liteai:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__LITEAI__ ??= {}
  const pending = window.__LITEAI__.deepLinks ?? []
  window.__LITEAI__.deepLinks = [...pending, ...urls]
  window.dispatchEvent(new CustomEvent(deepLinkEvent, { detail: { urls } }))
}

const listenForDeepLinks = () => {
  const startUrls = window.__LITEAI__?.deepLinks ?? []
...
const createPlatform = (): Platform => {
  ...
  const wslHome = async () => {
    if (os !== "windows" || !window.__LITEAI__?.wsl) return undefined
    return window.api.wslPath("~", "windows").catch(() => undefined)
  }

  const handleWslPicker = async <T extends string | string[]>(result: T | null): Promise<T | null> => {
    if (!result || !window.__LITEAI__?.wsl) return result
  ...
  return {
    ...
    async openDirectoryPickerDialog(opts?: { title?: string; multiple?: boolean }) {
    ...
    async openFilePickerDialog(opts?: { title?: string; multiple?: boolean }) {
    ...
    getWslEnabled: async () => {
      ...
      return window.__LITEAI__!.wsl ?? false
    },
    ...
    setDisplayBackend: async (backend: "auto" | "wayland") => {
...
render(() => {
  ...
  const [defaultServer] = createResource(() =>
    platform.getDefaultServer?.().then((url: ServerConnection.Key | null) => {

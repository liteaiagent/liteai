...
const TAURI_APP_IDS: Record<string, string> = {
  dev: "ai.liteai.desktop.dev",
  beta: "ai.liteai.desktop.beta",
  prod: "ai.liteai.desktop",
}
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.liteai.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `liteai.settings.dat` is special: it maps to the `liteai.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"liteai.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // liteai.settings.dat → the electron settings store ("liteai.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "liteai.global.dat").
  const storeName = filename === "liteai.settings.dat" ? "liteai.settings" : filename

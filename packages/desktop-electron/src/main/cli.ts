...
const CLI_INSTALL_DIR = ".liteai/bin"
const CLI_BINARY_NAME = "liteai"
...
export function getSidecarPath() {
  ...
  const path = app.isPackaged
    ? join(process.resourcesPath, `liteai-cli${suffix}`)
    : join(root, "../../resources", `liteai-cli${suffix}`)
...
export async function installCli(): Promise<string> {
  ...
  const tempScript = join(tmpdir(), "liteai-install.sh")
...
export function serve(hostname: string, port: number, password: string) {
  ...
  const env = {
    LITEAI_SERVER_USERNAME: "liteai",
    LITEAI_SERVER_PASSWORD: password,
  }
...
export function spawnCommand(args: string, extraEnv: Record<string, string>) {
  ...
  const envs = {
    ...base,
    LITEAI_EXPERIMENTAL_ICON_DISCOVERY: "true",
    LITEAI_EXPERIMENTAL_FILEWATCHER: "true",
    LITEAI_CLIENT: "desktop",
...
function buildCommand(args: string, env: Record<string, string>) {
  if (process.platform === "win32" && isWslEnabled()) {
    ...
    const script = [
      "set -e",
      'BIN="$HOME/.liteai/bin/liteai"',
      'if [ ! -x "$BIN" ]; then',
      `  curl -fsSL https://liteai.ai/install | bash -s -- --version ${shellEscape(version)} --no-modify-path`,
      "fi",

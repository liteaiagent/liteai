...
import { type AcquiredResources, AgentCleanup } from "@/agent/cleanup"
...
describe("Agent Cleanup Lifecycle", () => {
  ...
  test("runs all 11 steps successfully", async () => {
    ...
    expect((mockResources.contextMessages as unknown[]).length).toBe(0)
  ...
  test("passes the SC-007 baseline memory trace on rapid spawn/kill", async () => {
    const _memoryBaseline = process.memoryUsage().heapUsed
    ...
    if (lastTempResources) {
      expect((lastTempResources.contextMessages as unknown[]).length).toBe(0)
    }

...
test("plan agent denies edits except .liteai/plans/*", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      ...
      expect(PermissionNext.evaluate("edit", ".liteai/plans/foo.md", plan?.permission).action).toBe("allow")
    },
  })
})
...
test("permission config overrides default permissions", async () => {
  await using tmp = await tmpdir({
    config: {
      agent: {
        build: {
...
test("skill directories are allowed for external_directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".liteai", "skill", "perm-skill")
      await Bun.write(
...
`,
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        const build = await Agent.get("build")
        const skillDir = path.join(tmp.path, ".liteai", "skill", "perm-skill")
...
test("defaultAgent returns plan when no default_agent config", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
...
test("defaultAgent returns build when plan is disabled and default_agent not set", async () => {
  await using tmp = await tmpdir({
    config: {
      agent: {
        plan: { disable: true },
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const agent = await Agent.defaultAgent()
      // plan is disabled, so it should return build (next primary agent)

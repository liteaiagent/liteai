...
test("full integration test: plugin hooks.json -> run-hook.cmd -> session-start", async () => {
  ...
  await fs.writeFile(path.join(hooksDir, "hooks.json"), JSON.stringify(hooksJson))
...
`
  ...
  } catch {} // unix executable permission
  ...
  // biome-ignore lint/suspicious/noExplicitAny: deep nested plugin structure

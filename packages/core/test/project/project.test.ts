...
describe("Project.fromDirectory", () => {
  test("should handle git repository with no commits", async () => {
    ...
    expect(project.id).toBe(p.directoryId(tmp.path))
    expect(project.vcs).toBe("git")
    expect(project.worktree).toBe(tmp.path)

    const liteaiFile = path.join(tmp.path, ".git", "liteai")
    const fileExists = await Filesystem.exists(liteaiFile)
  ...
  test("should handle git repository with commits", async () => {
    ...
    expect(project).toBeDefined()
    expect(project.id).not.toBe(p.directoryId(tmp.path))
    expect(project.vcs).toBe("git")
    expect(project.worktree).toBe(tmp.path)
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  test("worktree should share project ID with main repo", async () => {
    ...
    try {
      ...
      const cache = path.join(tmp.path, ".git", "liteai")
...
describe("Project.update", () => {
  ...
  test("should emit GlobalBus event on update", async () => {
    ...
    // biome-ignore lint/suspicious/noExplicitAny: GlobalBus event shape is loosely typed

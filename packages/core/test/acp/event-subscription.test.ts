...
function createFakeAgent() {
  ...
  const agent = new ACP.Agent(connection, {
    sdk,
    defaultModel: { providerID: "liteai", modelID: "big-pickle" },
  } as unknown as ACPConfig)
  ...
  test("does not create additional event subscriptions on repeated loadSession()", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        await agent.loadSession({ sessionId, cwd, mcpServers: [] } as unknown as LoadSessionRequest)
        await agent.loadSession({ sessionId, cwd, mcpServers: [] } as unknown as LoadSessionRequest)
        await agent.loadSession({ sessionId, cwd, mcpServers: [] } as unknown as LoadSessionRequest)
        await agent.loadSession({ sessionId, cwd, mcpServers: [] } as unknown as LoadSessionRequest)

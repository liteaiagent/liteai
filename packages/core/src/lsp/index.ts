...
export namespace LSP {
  ...
  // Track all spawned LSP child processes for forced cleanup on exit
  ...
  async function getClients(file: string) {
    ...
    async function schedule(server: LSPServer.Info, root: string, key: string) {
      ...
      if (handle.process.pid === undefined) {
        s.broken.add(key)
        log.error(`LSP server ${server.id} process failed to start`)
        return undefined
      }
      children.add(handle.process)
      handle.process.on("exit", () => children.delete(handle.process))
  ...
  export async function workspaceSymbol(query: string, limit = 50) {
    return runAll((client) =>
      client.connection
        .sendRequest("workspace/symbol", {
          query,
        })
        .then((result) => (result as LSP.LspSymbol[]).filter((x) => kinds.includes(x.kind)))
        .then((result) => result.slice(0, limit))
        .catch(() => [] as LSP.LspSymbol[]),
    ).then((result) => result.flat().slice(0, limit))
  }

  function flatten(symbols: unknown[]): unknown[] {
    ...
    for (const sym of symbols) {
      ...
      if (Array.isArray(s.children)) {
        result.push(...flatten(s.children))
      }
    }
    return result
  }
  ...
  export async function documentSymbol(uri: string) {
    ...
    return run(file, (client) =>
      ...
      .then((result) => flatten(result.flat()) as (LSP.DocumentSymbol | LSP.LspSymbol)[])
  ...
  export async function codeAction(input: { file: string; line: number; character: number }) {
    const uri = pathToFileURL(input.file).href
    const normalized = Filesystem.normalizePath(input.file)
    return run(input.file, (client) => {
      const diags = (client.diagnostics.get(normalized) ?? []).filter(
        (d) => d.range.start.line <= input.line && d.range.end.line >= input.line,
      )
      return client.connection
  ...
  export async function fileDiagnostics(file: string) {
    const normalized = Filesystem.normalizePath(path.isAbsolute(file) ? file : path.resolve(Instance.directory, file))

...
export const Agent = z
  .object({
    ...
    tools: z
      .union([z.string(), z.array(z.string()), z.record(z.string(), z.boolean())])
      .optional()
      .describe("Allowed tools (Claude Code compat). Auto-mapped to permission allow rules."),
    disallowedTools: z
    ...
    skills: z.array(z.string()).optional().describe("Preloaded skills (Claude Code compat). Injected on agent init."),
    mcpServers: z
      .array(z.union([z.string(), z.record(z.string(), z.any())]))
      .optional()
      .describe("Agent-scoped MCP servers (Claude Code compat)."),
    effort: z
      .enum(["low", "medium", "high", "max"])
      .optional()
      .describe("Reasoning effort level (Claude Code compat)."),
    memory: z.enum(["user", "project", "local"]).optional().describe("MEMORY.md scope (Claude Code compat)."),
    background: z
      .boolean()
      .optional()
...
export const Server = z
  .object({
    ...
    mdnsDomain: z.string().optional().describe("Custom domain name for mDNS service (default: liteai.local)"),
...
export const Info = z
  .object({
    ...
    server: Server.optional().describe("Server configuration for liteai serve and web commands"),
    command: z
    ...
    hooks: z
      .record(
        ...
        z.array(
          z.object({
            matcher: z.string().optional().describe("Regex pattern to filter when this hook group fires"),
            hooks: z.array(
              z
                .object({
                  type: z.enum(["command", "prompt", "agent", "http"]),
                  command: z.string().optional().describe("Shell command to run (type: command)"),
                  prompt: z.string().optional().describe("Prompt text (type: prompt/agent)"),
                  url: z.string().optional().describe("URL to POST to (type: http)"),
                  headers: z.record(z.string(), z.string()).optional().describe("HTTP headers (type: http)"),
                  allowedEnvVars: z.array(z.string()).optional().describe("Env vars allowed for expansion in headers"),
                  timeout: z.number().optional().describe("Timeout in seconds (default: 600)"),
                  statusMessage: z.string().optional().describe("Message shown while hook runs"),
                  once: z.boolean().optional().describe("Only fire once per session"),
                  async: z.boolean().optional().describe("Run in background without blocking"),
                })
                .meta({ ref: "HookHandlerConfig" }),
            ),
          }),
        ),
      )
...
const schemaPath = path.join(Global.Path.config, "config.schema.json")

export async function schema() {
  const result = z.toJSONSchema(Info, {
    io: "input",
    override(ctx) {
      ...
      if (schema && typeof schema === "object" && "type" in schema && schema.type === "string" && schema?.default) {
        if (!schema.examples) schema.examples = [schema.default]
  ...
  }) as Record<string, unknown> & { allowComments?: boolean; allowTrailingCommas?: boolean }
  result.allowComments = true
  result.allowTrailingCommas = true
  await Filesystem.writeJson(schemaPath, result)
  return schemaPath
}

export const defaultConfig: Info = {

...
function generateRandomString(length: number): string {
  ...
  const limit = 256 - (256 % chars.length) // 198 (which is 3 * 66)
  let result = ""
...
<html>
  <head>
    <title>LiteAI - Codex Authorization Successful</title>
    <style>
      body {
        font-family:
          system-ui,
...
<html>
  <head>
    <title>LiteAI - Codex Authorization Failed</title>
    <style>
      body {
        font-family:
          system-ui,
...
export const CodexAuth: AuthProvider = {
    ...
    async loader(getAuth, provider) {
      ...
      if (!provider.models["gpt-5.3-codex"]) {
        const model = {
          ...
          // biome-ignore lint/suspicious/noExplicitAny: variant shape
      ...
      // biome-ignore lint/suspicious/noExplicitAny: model fields
  ...
  hooks: {
    "chat.headers": async (input, output) => {
      ...
      output.headers.originator = "liteai"

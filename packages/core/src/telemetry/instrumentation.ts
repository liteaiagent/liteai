...
export function isTelemetryEnabled() {
  return Boolean(process.env.LITEAI_ENABLE_TELEMETRY && process.env.LITEAI_ENABLE_TELEMETRY !== '0' && process.env.LITEAI_ENABLE_TELEMETRY !== 'false');
}
...
export async function shutdownTelemetry() {
  const timeoutMs = parseInt(process.env.LITEAI_OTEL_SHUTDOWN_TIMEOUT_MS || '2000');
...
export async function flushTelemetry(): Promise<void> {
  const timeoutMs = parseInt(process.env.LITEAI_OTEL_FLUSH_TIMEOUT_MS || '5000');

...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("liteai://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("liteai://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("liteai://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("liteai://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("liteai://open-project")).toBeUndefined()
    expect(parseDeepLink("liteai://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "liteai://open-project?directory=/a",
      "liteai://other?directory=/b",
      "liteai://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("liteai://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("liteai://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("liteai://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("liteai://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "liteai://new-session?directory=/a",
      "liteai://open-project?directory=/b",
      "liteai://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __LITEAI__: {
        deepLinks: ["liteai://open-project?directory=/a"],
      },
    } as unknown as Window & { __LITEAI__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["liteai://open-project?directory=/a"])
...
describe("layout workspace helpers", () => {
  ...
  test("normalizes forward and backward slashes in workspace key", () => {
    expect(workspaceKey("C:\\Users\\foo")).toBe(workspaceKey("C:/Users/foo"))
  })
  ...
  test("normalizePath converts backslashes and uppercases drive", () => {
    expect(normalizePath("C:\\Users\\foo")).toBe("C:/Users/foo")
    expect(normalizePath("c:\\Users\\foo")).toBe("C:/Users/foo")
    expect(normalizePath("/tmp/demo")).toBe("/tmp/demo")
  })

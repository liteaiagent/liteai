import type { Message, Session, TextPart, UserMessage } from "@liteai/sdk/client"
import { Avatar } from "@liteai/ui/avatar"
...
const LITEAI_PROJECT_ID = "4b0ea68d7af9a6031a7ffda7ad66e0cb83315750"
  ...
  return (
    <div class={`relative size-8 shrink-0 rounded ${props.class ?? ""}`}>
      <div class="size-full rounded overflow-clip">
        <Avatar
          fallback={name()}
          src={props.project.id === LITEAI_PROJECT_ID ? "https://liteai.ai/favicon.svg" : props.project.icon?.override}
...
function DialogDeleteSession(props: { session: Session; onDelete: (session: Session) => Promise<void> }) {
  ...
  const handle = async () => {
    await props.onDelete(props.session).catch((err: unknown) => {
      showToast({

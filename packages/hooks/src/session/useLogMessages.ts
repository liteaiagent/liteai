...
import type { AppState, LogPorts, Message } from '../types.js'

type AppStateSelector = <T>(selector: (state: AppState) => T) => T

/**
 * Hook that logs messages to the transcript.
 *
 * @param messages The current conversation messages
 * @param useAppState A selector hook for AppState (to be replaced by Context or Store)
 * @param ports LogPorts for platform-specific logging
 * @param ignore When true, messages will not be recorded to the transcript
 */
export function useLogMessages(messages: Message[], useAppState: AppStateSelector, ports: LogPorts, ignore = false) {
  ...
  const lastParentUuidRef = useRef<string | undefined>(undefined)
  ...
  useEffect(() => {
    ...
    if (isIncremental || wasFirstRender || isSameHeadShrink) {
      ...
      const last = ports.cleanMessagesForLogging(slice, messages).findLast(ports.isChainParticipant)
  ...
  }, [messages, ignore, teamContext?.teamName, teamContext?.selfAgentName, ports])
}

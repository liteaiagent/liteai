...
function pageToMessages(page: HistoryPage, ports: AssistantHistoryPorts): Message[] {
  const out: Message[] = []
  for (const ev of page.events) {
    const c = ports.convertSDKMessage(ev, {
      convertUserTextMessages: true,
      convertToolResults: true,
    }) as { type?: string; message?: Message } | undefined
    if (c?.type === 'message' && c.message) out.push(c.message)
  }
  return out
}
 ...
 */
export function useAssistantHistory({ config, setMessages, scrollRef, onPrepend, ports }: Props): Result {
  ...
  const mkSentinel = useCallback((text: string): SystemInformationalMessage => {
    return {
      type: 'system',
  ...
  const prepend = useCallback(
    (page: HistoryPage, isInitial: boolean) => {
      const msgs = pageToMessages(page, ports)
    ...
    [setMessages, ports, mkSentinel, scrollRef.current],
  )
  ...
  useEffect(() => {
    ...
    let cancelled = false
    void (async () => {
      const ctx = await ports.createHistoryAuthCtx(config.sessionId).catch(() => null)
      if (!ctx || cancelled) return
      ctxRef.current = ctx
      const page = await ports.fetchLatestEvents(ctx)
  ...
  const loadOlder = useCallback(async () => {
    ...
    try {
      const page = await ports.fetchOlderEvents(ctx, cursor)
      if (!page) {
  ...
  }, [enabled, prepend, setMessages, ports, mkSentinel])

...
/**
 * Hook that creates a double-press handler.
 * @param setPending Callback to indicate a press is pending
 * @param onDoublePress Callback for double press
  ...
  return useCallback(() => {
    ...
    if (isDoublePress) {
      ...
      timeoutRef.current = setTimeout(
        (setPending: (p: boolean) => void, timeoutRef: { current: ReturnType<typeof setTimeout> | undefined }) => {
          setPending(false)
          timeoutRef.current = undefined
        },

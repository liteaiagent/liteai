...
export function ThemeProvider({ children, mode: initialMode }: { children?: React.ReactNode; mode: "dark" | "light" }) {
  ...
  const [themes, setThemes] = useState<Record<string, ThemeJson>>({ liteai })
  ...
  useEffect(() => {
    if (active === "system" || themes[active]) return
    if (DEFAULT_THEME_NAMES.includes(active)) {
      Bun.file(path.join(import.meta.dir, "theme", `${active}.json`))
        .json()
        .then((data) => setThemes((prev) => ({ ...prev, [active]: data })))
        .catch(() => setActive("liteai"))
    }
  }, [active, themes])
  ...
  const value = useMemo(
    () => ({
      theme: values,
      selected: active,
      all: () => Array.from(new Set([...DEFAULT_THEME_NAMES, ...Object.keys(themes)])),
      syntax,
      subtleSyntax,
      mode: () => mode,

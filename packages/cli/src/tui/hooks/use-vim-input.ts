 ...
 * useVimInput hook
 *
 * Extends useTextInput with Vim-mode editing. Manages INSERT/NORMAL mode
 * transitions, command parsing, dot-repeat, and operator execution.
 *
 * Ported from MVP useVimInput.ts.
 */

import type { Key } from "@liteai/ink"
import React, { useCallback, useState } from "react"
import type { VimInputState, VimMode } from "../types/text-input"
import { Cursor } from "../util/cursor"
import { lastGrapheme } from "../util/intl"
import {
  executeIndent,
  executeJoin,
  executeOpenLine,
...
} from "../util/vim/operators"
import { type TransitionContext, transition } from "../util/vim/transitions"
import {
...
import { type UseTextInputProps, useTextInput } from "./use-text-input"
...
export function useVimInput(props: UseVimInputProps): VimInputState {
  ...
  function handleVimInput(rawInput: string, key: Key): void {
    ...
    // Backspace/Delete are only mapped in motion-expecting states
    const expectsMotion =

import { Box, Text, useDeclaredCursor } from "@liteai/ink"
...
export function ListItem({
  ...
  function renderIndicator(): ReactNode {
    ...
    if (isFocused) {
      return <ThemedText color="info">{figures.pointer}</ThemedText>
    }

    if (showScrollDown) {
      return <Text dim>{figures.arrowDown}</Text>
    }

    if (showScrollUp) {
      return <Text dim>{figures.arrowUp}</Text>
    }
  ...
  function getTextColor(): keyof ThemeColors | undefined {
    if (disabled) {
      return "textMuted"
    }
  ...
  return (
    <Box ref={cursorRef} flexDirection="column">
      <Box flexDirection="row" gap={1}>
        {renderIndicator()}
        {styled ? (
          <ThemedText color={textColor} dim={disabled}>
            {children}
          </ThemedText>
        ) : (
          children
        )}
        {isSelected && !disabled && <ThemedText color="success">{figures.tick}</ThemedText>}
      </Box>
      {description && (
        <Box paddingLeft={2}>
          <ThemedText color="textMuted">{description}</ThemedText>
        </Box>
      )}
    </Box>
  )

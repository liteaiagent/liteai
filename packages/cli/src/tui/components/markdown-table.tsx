...
import type { CliHighlight } from "../util/cliHighlight.ts"
import { formatToken, padAligned } from "../util/markdown.ts"
...
export function MarkdownTable({ token, highlight, forceWidth }: Props): React.ReactNode {
  ...
  function calculateMaxRowLines(): number {
    ...
    for (let i_1 = 0; i_1 < token.header.length; i_1++) {
      ...
      const wrapped = wrapText(content, columnWidths[i_1] ?? 3, {
        hard: needsHardWrap,
      })
    ...
    for (const row_1 of token.rows) {
      for (let i_2 = 0; i_2 < row_1.length; i_2++) {
        ...
        const wrapped_0 = wrapText(content_0, columnWidths[i_2] ?? 3, {
          hard: needsHardWrap,
        })
  ...
  function renderRowLines(
    ...
    const cellLines = cells.map((cell, colIndex_1) => {
      ...
      const width = columnWidths[colIndex_1] ?? 3
    ...
    for (let lineIdx = 0; lineIdx < maxLines_0; lineIdx++) {
      ...
      for (let colIndex_2 = 0; colIndex_2 < cells.length; colIndex_2++) {
        ...
        const width_0 = columnWidths[colIndex_2] ?? 3
  ...
  const maxLineWidth = Math.max(...tableLines.map((line: string) => stringWidth(stripAnsi(line))))

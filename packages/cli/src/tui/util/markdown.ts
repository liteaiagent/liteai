...
export function stripPromptXMLTags(text: string): string {
  return text.replace(/<antThinking>[\s\S]*?<\/antThinking>/g, "").replace(/<boltArtifact[\s\S]*?<\/boltArtifact>/g, "")
}
...
export function applyMarkdown(content: string, theme: ThemeColors, highlight: CliHighlight | null = null): string {
  configureMarked()
  return marked
...
export function formatToken(
  ...
  switch (token.type) {
    ...
    case "codespan": {
      // inline code
      return chalk.hex(theme.markdownCode || theme.text)(token.text)
    }
    case "em":
...
// No hyperlink support right now

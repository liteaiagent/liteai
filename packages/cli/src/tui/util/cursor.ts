import { stringWidth, wrapAnsi } from "@liteai/ink"
      ...
      .map((text, i) => {
        ...
        if (mask) {
          ...
          if (currentLine === allLines.length - 1) {
            ...
            const splitOffset = graphemes.length > visibleCount ? (graphemes[maskCount]?.index ?? 0) : 0
...
export class MeasuredText {
  ...
  private getLine(line: number): WrappedLine {
    ...
    const resolved = lines[clampedLine]
    if (!resolved) throw new Error(`No wrapped line at index ${clampedLine}`)
    return resolved
  }
  ...
  public getPositionFromOffset(offset: number): Position {
    ...
    if (!lastLine) return { line: 0, column: 0 }
    return {
      line,
  ...
  snapToGraphemeBoundary(offset: number): number {
    ...
    while (lo < hi) {
      const mid = (lo + hi + 1) >> 1
      const val = boundaries[mid]
      if (val !== undefined && val <= offset) lo = mid
      else hi = mid - 1
    }
    return boundaries[lo] ?? offset
  }
}

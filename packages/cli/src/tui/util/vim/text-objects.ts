...
import { isVimPunctuation, isVimWhitespace, isVimWordChar } from "../cursor"
import { getGraphemeSegmenter } from "../intl"
...
function findWordObject(
  ...
  for (let i = 0; i < graphemes.length; i++) {
    const g = graphemes[i]
    if (!g) continue
    const nextStart = i + 1 < graphemes.length ? (graphemes[i + 1]?.index ?? text.length) : text.length
  ...
  const offsetAt = (idx: number): number =>
    idx < graphemes.length ? (graphemes[idx]?.index ?? text.length) : text.length

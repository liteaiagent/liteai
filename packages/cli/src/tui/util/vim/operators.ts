...
import { firstGrapheme, lastGrapheme } from "../intl"
 ...
 */
function countCharInString(str: { indexOf(search: string, start?: number): number }, char: string, start = 0): number {
  let count = 0
...
export function executeLineOp(op: Operator, count: number, ctx: OperatorContext): void {
  ...
  ctx.recordChange({ type: "operator", op, motion: op[0] ?? op, count })
}

...
import { ModelsDev } from "liteai/provider/models"
import { Provider } from "liteai/provider/provider"
import { SessionPrompt } from "liteai/session/engine/index"
import { Session } from "liteai/session/index"
import { Message } from "liteai/session/message"
import type { SessionID } from "liteai/session/schema"
import { MessageID, PartID } from "liteai/session/schema"
...
const AGENT_USERNAME = "liteai-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/liteai.yml"
      ...
      async fn() {
        {
          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                "",
                "   Learn more about the GitHub agent - https://liteai.com/docs/github/#usage-examples",
              ].join("\n"),
            )
          }
          ...
          async function installGitHubApp() {
            ...
            // Open browser
            const url = "https://github.com/apps/liteai-agent"
            const command =
            ...
            async function getInstallation() {
              return await fetch(
                `https://api.liteai.ai/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`,
              )
                .then((res) => res.json())
...
jobs:
  liteai:
    if: |
      ...
      contains(github.event.comment.body, ' /liteai') ||
      startsWith(github.event.comment.body, '/liteai')
    ...
    steps:
      ...
      - name: Run liteai
        uses: liteaiagent/liteai/github@latest${envStr}
        with:
...
export const GithubRunCommand = cmd({
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.liteai.ai" : "https://liteai.ai"
      ...
      try {
        if (useGithubToken) {
          ...
          const actionToken = isMock ? (args.token as string) : await getOidcToken()
        ...
        console.log("liteai session", session.id)
        ...
        core.setFailed(msg instanceof Error ? msg : String(msg))
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.liteai.ai"
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env.MENTIONS || "/liteai,/oc")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to liteai...")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("liteai-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `liteai/${type}-${hex}-${timestamp}`
        }
        return `liteai/${type}${issueId}-${timestamp}`
      }
      ...
      async function addReaction(commentType?: "issue" | "pr_review") {
        ...
        return await octoRest.rest.reactions.createForIssue({
          owner,
          repo,
          issue_number: issueId as number,
          content: AGENT_REACTION,
        })
      }
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/liteai-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[liteai session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the liteai infrastructure after your response",

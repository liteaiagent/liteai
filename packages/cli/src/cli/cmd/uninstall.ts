...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall liteai and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using LiteAI!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes(Brand.marker) || content.includes(`${Brand.dir}/bin`)) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(`${Brand.dir}/bin`) || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(`${Brand.dir}/bin`)) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(Brand.dir))
    ) {
      continue
    }

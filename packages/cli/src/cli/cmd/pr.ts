...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run liteai",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for liteai session link in PR body
            if (prInfo?.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found liteai session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["liteai", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting liteai...")
        UI.println()

        // Launch liteai TUI with session ID if available
        const { spawn } = await import("node:child_process")
        const flags = sessionId ? ["-s", sessionId] : []
        const proc = spawn("liteai", flags, {
          stdio: "inherit",
          cwd: process.cwd(),
        })
        ...
        await new Promise<void>((resolve, reject) => {
          proc.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`liteai exited with code ${code}`))
          })

...
import { Session as SessionApi } from "@liteai/core/session/index"
...
async function getTerminalBackgroundColor(): Promise<"dark" | "light"> {
  ...
  return new Promise((resolve) => {
    ...
    const handler = (data: Buffer) => {
      ...
      const match = str.match(new RegExp(`${esc}]11;([^${bel}${esc}]+)`))
      if (match) {
        cleanup()
        const color = match[1]
...
function App() {
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.LITEAI_DISABLE_TERMINAL_TITLE) return

    if (route.data.type === "home") {
      renderer.setTerminalTitle("LiteAI")
      return
    }
  ...
  command.register(() => [
    ...
    },
    {
      ...
      value: "docs.open",
      onSelect: () => {
        open("https://liteai.com/docs").catch(() => {})
        dialog.clear()
      },
      category: "System",
    },
  ...
  sdk.event.on(Installation.Event.UpdateAvailable.type, (evt) => {
    toast.show({
      ...
      message: `LiteAI v${evt.properties.version} is available. Run 'liteai upgrade' to update manually.`,
      duration: 10000,
    })
  })

  // MCP.AuthRequired is a new domain event — cast to any to work around SDK types
  // until the SDK is regenerated to include this event type
  // biome-ignore lint/complexity/noBannedTypes: work around SDK types
  ;(sdk.event.on as Function)(MCP.AuthRequired.type, (evt: { properties: { message: string } }) => {
    toast.show({
  ...
  return (
    <>
      {/* biome-ignore lint/a11y/noStaticElementInteractions: TUI element, not web */}
      <box
...
function ErrorComponent(props: {
  ...
  const issueURL = new URL("https://github.com/liteaiagent/liteai/issues/new?template=bug-report.yml")
  ...
  issueURL.searchParams.set("liteai-version", Installation.VERSION)

...
export function PermissionPrompt(props: { request: PermissionRequest }) {
  ...
  // biome-ignore lint/suspicious/noExplicitAny: sync bounds mismatch
  const session = createMemo(() => sync.data.session.find((s: any) => s.id === props.request.sessionID))
          ...
          body={
            <Switch>
              <Match when={props.request.always.length === 1 && props.request.always[0] === "*"}>
                <TextBody title={`This will allow ${props.request.permission} until LiteAI is restarted.`} />
              </Match>
              <Match when={true}>
                <box paddingLeft={1} gap={1}>
                  <text fg={theme.textMuted}>This will allow the following patterns until LiteAI is restarted</text>
                  <box>
  ...
  return (
    ...
    >
      <box gap={1} paddingLeft={1} paddingRight={3} paddingTop={1} paddingBottom={1}>
        ...
        <box paddingLeft={1}>
          <text fg={theme.textMuted}>Tell LiteAI what to do differently</text>
        </box>
      </box>
      <box

  ...
  export async function colors(): Promise<{
    ...
    return new Promise((resolve) => {
      ...
      const handler = (data: Buffer) => {
        ...
        const bgMatch = str.match(new RegExp(`${esc}]11;([^${bel}${esc}]+)`))
        if (bgMatch) {
          background = parseColor(bgMatch[1])
        }

        // Match OSC 10 (foreground color)
        const fgMatch = str.match(new RegExp(`${esc}]10;([^${bel}${esc}]+)`))
        if (fgMatch) {
          foreground = parseColor(fgMatch[1])
        }

        // Match OSC 4 (palette colors)
        const paletteMatches = str.matchAll(new RegExp(`${esc}]4;(\\d+);([^${bel}${esc}]+)`, "g"))

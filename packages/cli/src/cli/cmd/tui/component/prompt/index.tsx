...
import { createEffect, createMemo, createSignal, type JSX, Match, on, onCleanup, onMount, Show, Switch } from "solid-js"
import "opentui-spinner/solid"
import path from "node:path"
import { MessageID, PartID } from "@liteai/core/session/schema"
import { Filesystem } from "@liteai/core/util/filesystem"
import { formatDuration } from "@liteai/core/util/format"
import { iife } from "@liteai/core/util/iife"
  ...
  command.register(() => [
    ...
    },
    {
      ...
      onSelect: (dialog: ReturnType<typeof useDialog>) => {
        dialog.replace(() => (
          <DialogStash
            onSelect={(entry: { input: string; parts: PromptInfo["parts"] }) => {
              input.setText(entry.input)
              setStore("prompt", { input: entry.input, parts: entry.parts })
              restoreExtmarksFromParts(entry.parts)
              input.gotoBufferEnd()
              ...
              <box flexShrink={0} flexDirection="row" gap={1}>
                ...
                <box flexDirection="row" gap={1} flexShrink={0}>
                  {(() => {
                    ...
                    return (
                      <Show when={retry()}>
                        {/* biome-ignore lint/a11y/noStaticElementInteractions: TUI box element, not HTML */}

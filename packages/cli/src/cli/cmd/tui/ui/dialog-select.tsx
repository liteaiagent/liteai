...
export function DialogSelect<T>(props: DialogSelectProps<T>) {
  ...
  return (
    <box gap={1} paddingBottom={1}>
      ...
      <Show when={props.footerContent || keybinds().length} fallback={<box flexShrink={0} />}>
        <box paddingRight={2} paddingLeft={4} flexDirection="row" gap={2} flexShrink={0} paddingTop={1}>
          <Show when={props.footerContent}>{props.footerContent}</Show>
            ...
            <For each={keybinds()}>
              {(item) => (
                <text>
                  ...
                  <span style={{ fg: theme.textMuted }}>{Keybind.format(item.keybind)}</span>
                </text>
              )}
            </For>
          </Show>

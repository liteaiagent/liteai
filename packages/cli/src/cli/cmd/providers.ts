    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (args.url) {
          ...
          const wellknown = (await fetch(`${url}/.well-known/liteai`).then((x) => x.json())) as {
        ...
        if (provider === "other") {
          ...
          prompts.log.warn(
            `This only stores a credential for ${provider} - you will need configure it in liteai.json, check the docs for examples.`,
          )
        }
        ...
        if (provider === "amazon-bedrock") {
          prompts.log.info(
            "Amazon Bedrock authentication priority:\n" +
              ...
              "Configure via liteai.json options (profile, region, endpoint) or\n" +
              "AWS environment variables (AWS_PROFILE, AWS_REGION, AWS_ACCESS_KEY_ID, AWS_WEB_IDENTITY_TOKEN_FILE).",
          )
        }

        if (provider === "google-code-assist") {
          prompts.log.info("Sign in with Google at https://idx.google.com to get a Code Assist API key")
        }

...
import { PermissionNext } from "@liteai/core/permission/next"
...
import { ToolRegistry } from "@liteai/core/tool/registry"
...
async function resolveTools(agent: Agent.Info, availableTools: Awaited<ReturnType<typeof getAvailableTools>>) {
  ...
  for (const tool of availableTools) {
    const rule = PermissionNext.evaluate("tool", tool.id, agent.permission ?? [])
    resolved[tool.id] = rule.action !== "deny"
  }
  return resolved
}

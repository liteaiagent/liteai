...
import { Brand } from "liteai/brand"
...
import { McpAuth } from "liteai/mcp/auth"
import { MCP } from "liteai/mcp/index"
import { McpOAuthProvider } from "liteai/mcp/oauth-provider"
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: liteai mcp add")
          return
        }
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .liteai/ subdirectory too)
  const candidates = [path.join(baseDir, `${Brand.config}.json`), path.join(baseDir, `${Brand.config}.jsonc`)]

  if (!global) {
    candidates.push(
      path.join(baseDir, Brand.dir, `${Brand.config}.json`),
      path.join(baseDir, Brand.dir, `${Brand.config}.jsonc`),
    )
  }
  ...
  // Default to settings.json if none exist
  return candidates[0]
}
      ...
      async fn() {
        ...
        if (type === "local") {
          const command = await prompts.text({
            ...
            placeholder: "e.g., liteai x @modelcontextprotocol/server-filesystem",
            validate: (x) => (x && x.length > 0 ? undefined : "Required"),
          })
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "liteai-debug", version: Installation.VERSION },
              },
              id: 1,
            }),
          })

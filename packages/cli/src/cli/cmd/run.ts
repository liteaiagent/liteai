...
import { Agent } from "liteai/agent/agent"
import { Flag } from "liteai/flag/flag"
import type { PermissionNext } from "liteai/permission/next"
import { Provider } from "liteai/provider/provider"
import { Server } from "liteai/server/server"
import type { CodeSearchTool } from "liteai/tool/codesearch"
import type { EditTool } from "liteai/tool/edit"
import type { GlobTool } from "liteai/tool/glob"
import type { GrepTool } from "liteai/tool/grep"
import type { ListTool } from "liteai/tool/ls"
import type { ReadTool } from "liteai/tool/read"
import type { RunCommandTool } from "liteai/tool/run_command"
import type { SkillTool } from "liteai/tool/skill"
import type { TaskTool } from "liteai/tool/task"
import type { TodoWriteTool } from "liteai/tool/todo"
import type { Tool } from "liteai/tool/tool"
import type { WebFetchTool } from "liteai/tool/webfetch"
import type { WebSearchTool } from "liteai/tool/websearch"
import type { WriteTool } from "liteai/tool/write"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run liteai with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running liteai server (e.g., http://localhost:9000)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to LITEAI_SERVER_PASSWORD)",
      })
      .option("dir", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    if (args.attach) {
      const headers = (() => {
        ...
        const username = process.env.LITEAI_SERVER_USERNAME ?? "liteai"
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createLiteaiClient({ baseUrl: "http://liteai.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})

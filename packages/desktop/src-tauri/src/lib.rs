...
async fn initialize(app: AppHandle) {
    ...
    let _ = ready_tx.send(ServerReadyData {
        url: url.clone(),
        username: Some("liteai".to_string()),
    ...
    let sqlite_done = needs_migration.then(|| {
        tracing::info!(
            path = %liteai_db_path().expect("failed to get db path").display(),
...
fn get_sidecar_port() -> u32 {
    option_env!("LITEAI_PORT")
        .map(|s| s.to_string())
        .or_else(|| std::env::var("LITEAI_PORT").ok())
...
fn sqlite_file_exists() -> bool {
    let Ok(path) = liteai_db_path() else {
        return true;
    };
...
fn liteai_db_path() -> Result<PathBuf, &'static str> {
    ...
    Ok(data_home.join("liteai").join("liteai.db"))
}

...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("liteai-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("liteai_lib=debug,liteai_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("liteai_lib=info,liteai_desktop=info,sidecar=info")
        }
    });

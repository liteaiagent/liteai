...
import {
	markCommentedDiffLines,
	markCommentedFileLines,
} from "../pierre/commented-lines";
import {
...
export type TextFileProps<T = unknown> = FileOptions<T> &
	SharedProps<T> & {
		mode: "text";
		file: FileContents;
...
export type DiffFileProps<T = unknown> = FileDiffOptions<T> &
	SharedProps<T> & {
		mode: "diff";
		before: FileContents;
		after: FileContents;
...
function diffMouseSide(node: HTMLElement) {
	...
	if (type === "change-deletion")
		return "deletions" satisfies DiffSelectionSide;
...
function TextViewer<T>(props: TextFileProps<T>) {
	...
	const adapter: ModeAdapter = {
		...
		buildDragSelection: () => {
			if (viewer.dragStart === undefined || viewer.dragEnd === undefined)
				return;
			return {
	...
	const lineCallbacks = createLineCallbacks({
		...
		onLineSelectionEnd: (range) => local.onLineSelectionEnd?.(range),
		onLineNumberSelectionEnd: (range) =>
			local.onLineNumberSelectionEnd?.(range),
	});
	...
	createEffect(() => {
		...
		renderViewer({
			...
			draw: (value) => {
				...
				value.render({
					file:
						typeof local.file.contents === "string"
							? local.file
							: { ...local.file, contents },
					lineAnnotations: [],
	...
	useAnnotationRerender<LineAnnotation<T>>({
		...
		current: () => instance,
		annotations: () =>
			(local.annotations as LineAnnotation<T>[] | undefined) ?? [],
	});
...
function DiffViewer<T>(props: DiffFileProps<T>) {
	...
	const adapter: ModeAdapter = {
		...
		buildDragSelection: () => {
			...
			if (dragEndSide && dragSide && dragEndSide !== dragSide)
				selected.endSide = dragEndSide;
			return selected;
		},
		buildClickSelection: () => {
			if (viewer.dragStart === undefined) return;
	...
	const large = createMemo(() => {
		const before =
			typeof local.before?.contents === "string" ? local.before.contents : "";
		const after =
			typeof local.after?.contents === "string" ? local.after.contents : "";
	...
	useAnnotationRerender<DiffLineAnnotation<T>>({
		...
		current: () => instance,
		annotations: () =>
			(local.annotations as DiffLineAnnotation<T>[] | undefined) ?? [],
	});

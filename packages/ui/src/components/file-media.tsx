...
export function FileMedia(props: {
	...
	const isBinary = createMemo(() => {
		...
		return isBinaryContent(media.current as unknown);
	});
	...
	const deleted = createMemo(() => {
		...
		return (
			!hasMediaValue(media.after as unknown) &&
			hasMediaValue(media.before as unknown)
		);
	});
	...
	const [loaded] = createResource(request, async (input) => {
		return input.readFile(input.path).then(
			(result) => {
				const src = dataUrlFromMediaValue(result as unknown, input.kind);
				if (!src) {
	...
	const svgSource = createMemo(() => {
		...
		return svgTextFromValue(media.current as unknown);
	});
	const svgSrc = createMemo(() => {
		const media = cfg();
		if (!media || kind() !== "svg") return;
		return dataUrlFromMediaValue(media.current as unknown, "svg");
	});
	const svgInvalid = createMemo(() => {
		const media = cfg();
		if (!media || kind() !== "svg") return;
		if (svgSource() !== undefined) return;
		if (!hasMediaValue(media.current as unknown)) return;
	...
	const kindLabel = (value: "image" | "audio") =>
		i18n.t(
			value === "image" ? "ui.fileMedia.kind.image" : "ui.fileMedia.kind.audio",
		);
	...
	return (
		<Switch>
			...
			<Match when={kind() === "svg"}>
				{(() => {
					if (svgSource() === undefined && svgSrc() == null)
						return props.fallback();
				...
				<div class="flex min-h-56 flex-col items-center justify-center gap-2 px-6 py-10 text-center">
					...
					<div class="text-14-regular text-text-weak">
						{(() => {
							const path = cfg()?.path;
							if (!path)
								return i18n.t("ui.fileMedia.binary.description.default");

			...
			>
              ...
              [data-reduced-motion="true"] *::after {
				...
				<div style={{ display: "flex", gap: "8px", "flex-wrap": "wrap" }}>
					<button
						type="button"
						onClick={() => (active() ? stopSim() : startSim())}
						style={btn(active())}
					>
				...
				<div style={{ display: "flex", gap: "8px", "flex-wrap": "wrap" }}>
					<button
						type="button"
						onClick={() => setState("reads", (value) => value + 1)}
						style={smallBtn()}
					>
						+ read
					</button>
					<button
						type="button"
						onClick={() => setState("searches", (value) => value + 1)}
						style={smallBtn()}
					>
						+ search
					</button>
					<button
						type="button"
						onClick={() => setState("lists", (value) => value + 1)}
						style={smallBtn()}
					>
						+ list
					</button>
				...
				>
					motion: {reducedMotion() ? "reduced" : "normal"} · active:{" "}
					{active() ? "true" : "false"} · reads: {reads()} · searches:{" "}
					{searches()} · lists: {lists()}
				</div>
			</div>
		);
	},

...
import type { SessionStatus } from "@liteai-ai/sdk";
import type {
	AssistantMessage,
	FileDiff,
...
export function SessionTurn(
	...
	const allMessages = createMemo(() =>
		list(data.store.message?.[props.sessionID], emptyMessages),
	);
	...
	const messageIndex = createMemo(() => {
		...
		const index = result.found
			? result.index
			: messages.findIndex((m) => m.id === props.messageID);
		if (index < 0) return -1;
	...
	const showReasoningSummaries = createMemo(
		() => props.showReasoningSummaries ?? true,
	);
	...
	const assistantVisible = createMemo(() =>
		assistantMessages().reduce((count, message) => {
			...
			return (
				count +
				parts.filter(
					(part) => partState(part, showReasoningSummaries()) === "visible",
				).length
			);
		}, 0),
	);
	const _assistantTailVisible = createMemo(() =>
		assistantMessages()
	...
	const reasoningHeading = createMemo(() =>
		assistantMessages()
			.flatMap((message) => list(data.store.part?.[message.id], emptyParts))
			.filter(
				(part): part is PartType & { type: "reasoning"; text: string } =>
					part.type === "reasoning",
			)
	...
	return (
		<div data-component="session-turn" class={props.classes?.root}>
			...
			>
				{/* biome-ignore lint/a11y/noStaticElementInteractions: captures interaction to stop auto-scroll */}
				{/* biome-ignore lint/a11y/useKeyWithClickEvents: captures interaction to stop auto-scroll */}

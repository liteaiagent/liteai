...
import type { FileContent, FileDiff } from "@liteai-ai/sdk";
...
export const SessionReview = (props: SessionReviewProps) => {
	...
	const diffs = createMemo(
		() => new Map(props.diffs.map((diff) => [diff.file, diff] as const)),
	);
	const diffStyle = () =>
		props.diffStyle ?? (props.split ? "split" : "unified");
	...
	createEffect(() => {
		...
		untrack(() => {
			...
			const scrollTo = (attempt: number) => {
				...
				const next =
					root.scrollTop + offset - rootRect.height / 2 + targetRect.height / 2;
	...
	return (
		...
		>
			...
			<ScrollView
				...
				viewportRef={(el) => {
					scroll = el;
					props.scrollRef?.(el as HTMLDivElement);
				}}
				onScroll={
					props.onScroll as JSX.EventHandlerUnion<HTMLDivElement, Event>
				}
				classList={{
								...
								<For each={files()}>
									{(file) => {
										...
										const comments = createMemo(() =>
											(props.comments ?? []).filter((c) => c.file === file),
										);
										...
										const beforeText = () =>
											typeof item().before === "string" ? item().before : "";
										const afterText = () =>
											typeof item().after === "string" ? item().after : "";
										const changedLines = () =>
										...
										const commentsUi =
											createLineCommentController<SessionReviewComment>({
												...
												state: {
													...
													setOpened: (id) =>
														setStore("opened", id ? { file, id } : null),
												...
												renderCommentActions: props.lineCommentActions
													? (_comment, controls) => (
															<ReviewCommentMenu
																labels={
																	props.lineCommentActions as SessionReviewCommentActions
																}
										...
										return (
											<Accordion.Item
												...
												data-selected={
													props.focusedFile === file ? "" : undefined
												}
											>
															...
															<div data-slot="session-review-trigger-actions">
																<Switch>
																	...
																	<Match when={isDeleted()}>
																		...
																		>
																			{i18n.t(
																				"ui.sessionReview.change.removed",
																			)}
																		</span>
																	</Match>
																	<Match when={!!mediaKind()}>
																		...
																		>
																			{i18n.t(
																				"ui.sessionReview.change.modified",
																			)}
																		</span>
																	</Match>
																	<Match when={true}>
																...
																<Match when={true}>
																	<Dynamic
																		...
																		onLineNumberSelectionEnd={
																			commentsUi.onLineNumberSelectionEnd
																		}
																		annotations={commentsUi.annotations()}
																		renderAnnotation={
																			commentsUi.renderAnnotation
																		}
																		renderHoverUtility={
																			props.onLineComment
																				? commentsUi.renderHoverUtility
																				: undefined
																		}

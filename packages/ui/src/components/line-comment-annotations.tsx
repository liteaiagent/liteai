...
export function createLineCommentAnnotationRenderer<T>(props: {
			...
			if (active.kind === "comment") {
				const view = createMemo(() => {
					...
					if (next.kind !== "comment")
						return props.renderComment(active.comment);
	...
	const render = <A extends { metadata: LineCommentAnnotationMeta<T> }>(
		annotation: A,
	) => {
	...
	const reconcile = <A extends { metadata: LineCommentAnnotationMeta<T> }>(
		annotations: A[],
	) => {
		const next = new Set(
...
export function createLineCommentController<T extends LineCommentShape>(
	...
	renderAnnotation: ReturnType<
		typeof createManagedLineCommentAnnotationRenderer<T>
	>["renderAnnotation"];
...
export function createLineCommentAnnotations<T>(
	props:
		| LineCommentAnnotationsProps<T>
		| LineCommentAnnotationsWithSideProps<T>,
) {
	...
	return (getHoveredLine: () => HoverCommentLine | undefined) =>
		createHoverCommentUtility({
			...
			onSelect: (hovered) => {
				...
				if (
					current &&
					lineInSelectedRange(current, hovered.lineNumber, hovered.side)
				) {

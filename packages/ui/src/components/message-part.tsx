...
import { getDirectory as _getDirectory, getFilename } from "@liteai/util/path";
import type {
	AgentPart,
	AssistantMessage,
	FilePart,
			...
			new Map(
				props.messages.map(
					(message) =>
						[
							message.id,
							index(list(data.store.part?.[message.id], emptyParts)),
						] as const,
				),
			),
	);
						...
						<Match when={entryType() === "context"}>
							{(() => {
								...
								const busy = createMemo(
									() => props.working && last() === entryAccessor().key,
								);
...
function contextToolSummary(parts: ToolPart[]) {
	...
	const search = parts.filter(
		(part) => part.tool === "glob" || part.tool === "grep",
	).length;
					...
					<Index each={props.parts}>
						{(partAccessor) => {
							const trigger = createMemo(() =>
								contextToolTrigger(partAccessor(), i18n),
							);
...
export function UserMessageDisplay(props: {
	...
	const command = createMemo(
		() => textPart()?.metadata?.command as string | undefined,
	);

	const text = createMemo(() => {
		if (command())
			return (textPart()?.metadata?.arguments as string | undefined) || "";
			...
			<Show when={text() || command()}>
				...
				<div data-slot="user-message-copy-wrapper">
					...
					<Show when={props.actions?.fork}>
						<Tooltip
							value={i18n.t("ui.message.forkMessage")}
							placement="top"
							gutter={4}
						>
							<IconButton
					...
					<Show when={props.actions?.revert}>
						<Tooltip
							value={i18n.t("ui.message.revertMessage")}
							placement="top"
							gutter={4}
						>
							<IconButton
	...
	const segments = createMemo(() => {
		...
		const allRefs: { start: number; end: number; type: "file" | "agent" }[] = [
			...props.references
				.filter(
					(r) =>
						r.source?.text?.start !== undefined &&
						r.source?.text?.end !== undefined,
				)
				.map((r) => ({
...
PART_MAPPING.tool = function ToolPartDisplay(props) {
	...
	const render = createMemo(
		() => ToolRegistry.render(part().tool) ?? GenericTool,
	);
							...
							<span data-slot="reasoning-trigger-label">
								...
								<Show when={!done()}>
									...
									<Show when={elapsedLabel()}>
										<span data-slot="reasoning-trigger-elapsed">
											{elapsedLabel()}
										</span>
									</Show>
								</Show>
...
ToolRegistry.register({
	...
	render(props) {
		...
		const questions = createMemo(
			() => (props.input.questions ?? []) as QuestionInfo[],
		);
		const answers = createMemo(
			() => (props.metadata.answers ?? []) as QuestionAnswer[],
		);

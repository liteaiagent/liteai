...
export function AnimatedCountLabel(props: {
	...
	const splitSuffix = createMemo(
		() =>
			...
			(one().after.startsWith(other().after) ||
				other().after.startsWith(one().after)),
	);
	const before = createMemo(() =>
		splitSuffix() ? one().before : active().before,
	);
	const stem = createMemo(() =>
		splitSuffix() ? suffix().stem : active().after,
	);
		...
		<span data-component="tool-count-label" class={props.class}>
			...
			<span data-slot="tool-count-label-word">
				...
				<span
					...
					data-active={showTail() ? "true" : "false"}
				>

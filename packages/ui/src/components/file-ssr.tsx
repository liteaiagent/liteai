...
import { type DiffFileProps, File, type FileProps } from "./file";
...
function DiffSSRViewer<T>(props: SSRDiffFileProps<T>) {
	...
	let sharedVirtualizer:
		| NonNullable<ReturnType<typeof acquireVirtualizer>>
		| undefined;
	...
	const setSelectedLines = (
		range: DiffFileProps<T>["selectedLines"],
		attempt = 0,
	) => {
		...
		>
			<Dynamic component={DIFFS_TAG_NAME} ref={fileDiffRef} id="ssr-diff">
				<Show when={isServer}>
					<template
						shadowrootmode="open"
						innerHTML={local.preloadedDiff.prerenderedHTML}
					/>
				</Show>
			</Dynamic>
		</div>

...
export function Select<T>(
	props: SelectProps<T> & Omit<ButtonProps, "children">,
) {
	...
	const keyFor = (item: T) =>
		local.value ? local.value(item) : (item as string);
	...
	return (
		...
		<Kobalte<T, { category: string; options: T[] }>
			...
			placement={
				local.triggerVariant === "settings" ? "bottom-end" : "bottom-start"
			}
			gutter={4}
			value={local.current}
			options={grouped()}
		...
		>
			...
			>
				...
				<Kobalte.Icon data-slot="select-select-trigger-icon">
					<Icon
						name={
							local.triggerVariant === "settings" ? "selector" : "chevron-down"
						}
						size="small"
					/>
				</Kobalte.Icon>

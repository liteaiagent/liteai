import { type FilteredListProps, useFilteredList } from "@liteai/ui/hooks";
...
export function List<T>(
	props: ListProps<T> & { ref?: (ref: ListRef) => void },
) {
	const i18n = useI18n();
	...
	const setInternalFilter = (value: string) =>
	...
	function GroupHeader(groupProps: {
		...
		createEffect(() => {
			...
			const handler = () => {
				...
				setState(
					"stuck",
					rect.top <= scrollRect.top + 1 && scroll.scrollTop > 0,
				);
			};
	...
	const emptyMessage = () => {
		if (grouped.loading)
			return props.loadingMessage ?? i18n.t("ui.list.loading");
			...
			<div ref={setScrollRef} data-slot="list-scroll">
				...
				>
					<For each={grouped.latest}>
						{(group, groupIndex) => {
							const isLastGroup = () =>
								groupIndex() === grouped.latest.length - 1;
							return (
										...
										<For each={group.items}>
											{(item, i) => {
												...
												if (props.itemWrapper)
													return props.itemWrapper(item, node);
												return node;
											}}
										</For>

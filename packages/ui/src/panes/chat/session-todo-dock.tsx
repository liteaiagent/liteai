...
import { AnimatedNumber } from "@liteai/ui/animated-number"
import { Checkbox } from "@liteai/ui/checkbox"
import { DockTray } from "@liteai/ui/dock-surface"
import { IconButton } from "@liteai/ui/icon-button"
import { useSpring } from "@liteai/ui/motion-spring"
import { TextReveal } from "@liteai/ui/text-reveal"
...
export function SessionTodoDock(props: {
  ...
  const progress = createMemo(() =>
    language
      .t("session.todo.progress", { done: `\uE001done\uE001`, total: `\uE001total\uE001` })
      .split("\uE001")
      .filter((s) => s !== ""),
  )
    ...
    >
      <div ref={contentRef}>
        <button
          ...
          type="button"
          class="pl-3 pr-2 py-2 flex items-center gap-2 overflow-visible w-full text-left"
          aria-label={label()}
          onClick={toggle}
          onKeyDown={(event) => {

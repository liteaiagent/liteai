...
export function findCodeSelectionSide(
	node: Node | null,
): SelectedLineRange["side"] {
	const el = findElement(node);
	if (!el) return;
...
export function readShadowLineSelection(opts: {
	...
	const selection =
		(
			opts.root as unknown as { getSelection?: () => Selection | null }
	...
	return {
		range,
		text:
			opts.preserveTextSelection && domRange
				? toRange(domRange).cloneRange()
				: undefined,
	};
}

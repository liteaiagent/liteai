...
export function markCommentedDiffLines(
	root: ShadowRoot,
	...
	const annotations = Array.from(
		diffs.querySelectorAll("[data-line-annotation]"),
	).filter((node): node is HTMLElement => node instanceof HTMLElement);

	for (const range of ranges) {
		const start = diffRowIndex(
			root,
			split,
			range.start,
			range.side as CommentSide | undefined,
		);
...
export function markCommentedFileLines(
	...
	const annotations = Array.from(
		root.querySelectorAll("[data-line-annotation]"),
	).filter((node): node is HTMLElement => node instanceof HTMLElement);
		...
		for (let line = start; line <= end; line++) {
			const nodes = Array.from(
				root.querySelectorAll(
					`[data-line="${line}"], [data-column-number="${line}"]`,
				),
			);

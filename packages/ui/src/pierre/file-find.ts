...
function clearHighlightFind() {
	const api = (
		globalThis as { CSS?: { highlights?: { delete: (name: string) => void } } }
	).CSS?.highlights;
	if (!api) return;
	api.delete("liteai-find");
	api.delete("liteai-find-current");
}
...
export function createFileFind(opts: CreateFileFindOptions) {
	...
	const renderOverlay = () => {
		...
		for (let i = 0; i < hits.length; i++) {
			...
			for (const rect of Array.from(range.getClientRects())) {
				...
				if (active)
					mark.style.boxShadow = "inset 0 0 0 1px var(--border-warning-base)";
	...
	const syncOverlayScroll = () => {
		...
		const next = root
			...
			: [];
		if (
			next.length === overlayScroll.length &&
			next.every((el, i) => el === overlayScroll[i])
		)
			return;
		...
		overlayScroll = next;
		for (const el of overlayScroll)
			el.addEventListener("scroll", scheduleOverlay, { passive: true });
	};
	...
	const positionBar = () => {
		...
		const title = parseFloat(
			getComputedStyle(root).getPropertyValue("--session-title-height"),
		);
	...
	const scan = (root: ShadowRoot, value: string) => {
		...
		const cols = Array.from(
			root.querySelectorAll(
				"[data-content] [data-line], [data-column-content]",
			),
	...
	const setHighlights = (ranges: Range[], currentIndex: number) => {
		const api = (
			globalThis as unknown as { CSS?: { highlights?: any }; Highlight?: any }
		).CSS?.highlights;
		...
		if (active) api.set("liteai-find-current", new Highlight(active));

		const rest = ranges.filter((_, i) => i !== currentIndex);
		if (rest.length > 0) api.set("liteai-find", new Highlight(...rest));
		return true;
	};

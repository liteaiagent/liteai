...
function hostForNode(node: unknown) {
  ...
  for (const host of hosts) {
    ...
    if (el?.isConnected && el.contains(node)) return host
  }
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    type HighlightType = new (...ranges: Range[]) => unknown
    type HighlightsAPI = { delete: (name: string) => void; set: (name: string, value: unknown) => void }
    const api = (globalThis as unknown as { CSS?: { highlights?: HighlightsAPI } }).CSS?.highlights
    const Highlight = (globalThis as unknown as { Highlight?: HighlightType }).Highlight

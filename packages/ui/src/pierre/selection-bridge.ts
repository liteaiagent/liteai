...
type SelectionKey =
	| "ui.sessionReview.selection.line"
	| "ui.sessionReview.selection.lines";
...
export function formatSelectedLineLabel(
	...
	if (start === end)
		return t("ui.sessionReview.selection.line", { line: start });
...
export function restoreShadowTextSelection(
...
export function createLineNumberSelectionBridge() {
	...
	return {
		...
		track(buttons: number, next: number | undefined) {
			...
			if (next !== undefined && line !== undefined && next !== line)
				moved = true;
			return true;
		},
		finish() {

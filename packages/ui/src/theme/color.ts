...
export function generateNeutralScale(
	...
	if (ink) {
		...
		const bg = isDark
			? sink(
					clamp(
						0.19 + Math.max(0, base.l - 0.12) * 0.33 + base.c * 1.95,
						0.17,
						0.27,
					),
				)
			: base.l < 0.82
				...
				: lift(
						clamp(
							0.1 + base.c * 3.2 + Math.max(0, 0.95 - base.l) * 0.35,
							0.1,
							0.28,
						),
					);
		const steps = isDark
			? [
					0, 0.018, 0.039, 0.064, 0.097, 0.143, 0.212, 0.31, 0.46, 0.649, 0.845,
					0.984,
				]
			: [
					0, 0.022, 0.042, 0.068, 0.102, 0.146, 0.208, 0.296, 0.432, 0.61, 0.81,
					0.965,
				];

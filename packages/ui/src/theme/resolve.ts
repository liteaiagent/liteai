...
export function resolveThemeVariant(
	...
	const amber = generateScale(
		shift(
			colors.warning,
			isDark ? { h: -16, l: -0.058, c: 1.14 } : { h: -22, l: -0.082, c: 0.94 },
		),
		isDark,
	);
	const blue = generateScale(
		shift(colors.interactive, { h: -12, l: 0.128, c: 1.12 }),
		isDark,
	);
	const diffAdd = generateScale(
		colors.diffAdd ??
			shift(colors.success, {
				c: isDark ? 0.7 : 0.55,
				l: isDark ? -0.18 : 0.14,
			}),
		isDark,
	...
	const diffDelete = generateScale(
		colors.diffDelete ??
			shift(colors.error, { c: isDark ? 0.82 : 0.7, l: isDark ? -0.08 : 0.08 }),
		isDark,
	);
	...
	const body = tint
		? shift(ink, {
				l: isDark
					? Math.max(0, 0.88 - tint.l) * 0.4
					: -Math.max(0, tint.l - 0.18) * 0.24,
				c: isDark ? 1.04 : 1.02,
			})
		: undefined;
	...
	const modified = () => {
		...
		const delta = Math.abs(
			((((deleteHue - warningHue) % 360) + 540) % 360) - 180,
		);
	...
	const borderTone = (light: number, dark: number) =>
		alphaTone(
			ink,
			isDark
				? Math.min(1, dark + 0.024 + (colors.compact ? 0.08 : 0))
				: Math.min(1, light + 0.024),
		);
	...
	tokens["surface-base-interactive-active"] = withAlpha(
		interactive[2],
		0.3,
	) as ColorValue;
	...
	tokens["surface-raised-stronger-hover"] = isDark
		? neutralAlpha[6]
		: "#ffffff";
	...
	tokens["border-base"] = colors.compact
		? borderTone(0.22, 0.16)
		: neutralAlpha[6];
	tokens["border-hover"] = colors.compact
		? borderTone(0.28, 0.2)
		: neutralAlpha[7];
	tokens["border-active"] = colors.compact
		? borderTone(0.34, 0.24)
		: neutralAlpha[8];
	tokens["border-selected"] = withAlpha(
		interactive[8],
		isDark ? 0.9 : 0.99,
	) as ColorValue;
	tokens["border-disabled"] = colors.compact
		? borderTone(0.18, 0.12)
		: neutralAlpha[7];
	tokens["border-focus"] = colors.compact
		? borderTone(0.34, 0.24)
		: neutralAlpha[8];
	...
	tokens["border-strong-hover"] = colors.compact
		? borderTone(0.4, 0.28)
		: neutralAlpha[7];
	tokens["border-strong-active"] = colors.compact
		? borderTone(0.46, 0.32)
		: neutralAlpha[isDark ? 7 : 6];
	tokens["border-strong-selected"] = withAlpha(
		interactive[5],
		0.6,
	) as ColorValue;
	tokens["border-strong-disabled"] = colors.compact
		? borderTone(0.14, 0.1)
		: neutralAlpha[5];
	...
	tokens["border-weak-active"] = colors.compact
		? borderTone(0.22, 0.16)
		: neutralAlpha[isDark ? 7 : 6];
	tokens["border-weak-selected"] = withAlpha(
		interactive[4],
		isDark ? 0.6 : 0.5,
	) as ColorValue;
	tokens["border-weak-disabled"] = colors.compact
		? borderTone(0.08, 0.06)
		: neutralAlpha[5];
	tokens["border-weak-focus"] = colors.compact
		? borderTone(0.22, 0.16)
		: neutralAlpha[isDark ? 7 : 6];
	tokens["border-weaker-base"] = colors.compact
	...
	tokens["icon-base"] =
		colors.compact && !isDark ? tokens["text-weak"] : neutral[isDark ? 9 : 8];
	tokens["icon-hover"] =
		colors.compact && !isDark ? tokens["text-base"] : neutral[10];
	tokens["icon-active"] =
		colors.compact && !isDark ? tokens["text-strong"] : neutral[11];
	tokens["icon-selected"] =
		colors.compact && !isDark ? tokens["text-strong"] : neutral[11];
	...
	if (colors.compact) {
		...
		tokens["syntax-punctuation"] = isDark
			? "var(--text-weak)"
			: "var(--text-base)";
	...
	if (
		colors.compact &&
		"text-weak" in overrides &&
		!("text-weaker" in overrides)
	) {
		const weak = tokens["text-weak"];
		if (weak.startsWith("#")) {
			tokens["text-weaker"] = shift(weak as HexColor, {
				l: isDark ? -0.12 : 0.12,
				c: 0.75,
			});
		} else {
...
function generateNeutralAlphaScale(
	neutralScale: HexColor[],
	isDark: boolean,
): HexColor[] {
	const alphas = isDark
		? [
				0.038, 0.066, 0.1, 0.142, 0.19, 0.252, 0.334, 0.446, 0.58, 0.718, 0.854,
				0.985,
			]
